...
namespace core5Rx17
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            #region 允許Cookie-based 或 JWT-based 身份驗證, 以Cookie-based為預設
            ...
            /// 本機快取，使可注入：IMemoryCache。將只應用於取代Session存放登入者資訊。
        ...
        /// </summary>
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ISysEnv sysenv)
        {
            ...
            app.Use(next => context =>
            {
                ...
                Debug.WriteLine($"{method} {path}", "TRACE");

                //## prefix
                // 只有入口畫面可以取得 Antiforgery Token
                //if (method == "GET" && String.Equals(path, "/"))

                // 只有POST與入口畫面可以取得 Antiforgery Token
                if (method == "POST" || method == "GET" && String.Equals(path, "/"))
                {
                    // 取得 Antiforgery Token
                    sysenv.GetAndStoreAntiforgeryTokens(context);
                    Debug.WriteLine($"\t=> refresh AntiforgeryToken", "TRACE");
                }
